...
#获取场景图
#场景图是一个描述场景中节点之间连接关系的数据结构。方法的实现如下：
#节点之间关系 那我只有一个节点 那这个图其实无关紧要
    ...
    def present_nodes(self,
        ...
        for node in self.nodes:
            ...
            if type is None or node.type == type:
                # 对于其余情况，计算节点的历史时间步和未来时间步范围，
                # 并根据 timesteps、min_history_timesteps 和 min_future_timesteps 来判断该节点是否在给定时间步存在。
                # 将符合条件的节点加入到对应时间步的节点列表中。
