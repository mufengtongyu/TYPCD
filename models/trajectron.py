...
# *******z = self.encoder.get_latent(batch, node_type)
    def get_latent(self, batch, node_type):
        ...
        gph_data_x = gph_data_x.to(self.device)
        ...
        #这里接受的是12的长度 所以不能分开写
        x_gph,encoded_age,encoded_env_data = model.get_latent(inputs=x, #torch.Size([256, 8, 12])       x是什么 历史的8个时刻的节点坐标速度加速度 调用节点模型的get_latent方法，传递所需的输入和标签，包括输入位置、输入速度、历史索引、目标位置、目标速度、邻居数据、邻居边缘值、机器人轨迹、地图和预测时域。
                                inputs_st=x_st_t, #torch.Size([256, 8, 12])
                                first_history_indices=first_history_index, #torch.Size([256])
                                labels=y, #torch.Size([256, 4, 4])
                                labels_st=y_st_t, #torch.Size([256, 4, 4])
                                neighbors=restore(neighbors_data_st), #256
                                neighbors_edge_value=restore(neighbors_edge_value), #256
