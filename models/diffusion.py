...
class TransformerConcatLinear(Module):
    ...
    def forward(self, x, beta, context, encoded_age, encoded_env_data):
        ...
        for block in self.blocks:
            ...
            # Keep the condition tensor 3D so it can broadcast across the sequence
            # dimension inside AdaLayerNorm. Squeezing removed the time axis and
            # led to shape mismatches when applying the adaptive shift/scale.
            token = block(token, cond)
