...
#设置当前环境的方法。它根据输入的环境创建节点模型，并将其添加到节点模型字典中。
...
#进行一步退火的方法。可以选择指定节点类型或对所有节点类型进行退火。
    ...
    # 计算训练损失的方法。给定一个批次数据和节点类型，它调用对应的节点模型来计算训练损失。
    def train_loss(self, batch, node_type):
        ...
        # Run forward pass
        model = self.node_models_dict[node_type] #??什么模型
    ...
    def predict(self,
        ...
        for node_type in self.env.NodeType:
            ...
            model = self.node_models_dict[node_type] #是什么
