...
class WindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads):
        ...
        relative_position_index = self._build_relative_position_index(window_size)
...
class SwinBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.0):
        ...
        self.attn_mask = None
        if shift_size > 0:
            self.attn_mask = self._create_mask(input_resolution, window_size, shift_size)

    def _create_mask(self, input_resolution, window_size, shift_size):
    ...
    def forward(self, x, H, W):
        ...
        attn_windows = self.attn(x_windows, mask=self.attn_mask.to(x_windows.device) if self.attn_mask is not None else None)
...
class SwinStage(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, downsample=True):
        super().__init__()
...
class SwinTinyBackbone(nn.Module):
    def __init__(self, in_chans=2, window_size=7, embed_dim=96,
        ...
        for i, depth in enumerate(depths):
            ...
            if i == 0:
                stage_resolution = (0, 0)
            else:
                stage_resolution = (input_res[0] // 2, input_res[1] // 2)
            stage = SwinStage(dim=dims[i], input_resolution=stage_resolution,
                              depth=depth, num_heads=num_heads[i],
                              window_size=window_size, downsample=downsample)
            self.stages.append(stage)
            input_res = stage_resolution
    ...
    def forward(self, x):
        ...
        patch_size = self.patch_embed.proj.kernel_size[0]
        num_down = sum(1 for s in self.stages if s.downsample is not None)
        base = patch_size * self.window_size * (2 ** num_down)
        ...
        # 第一次&唯一一次 patch_embed：从 2 通道图像 → token 序列
        ...
        for stage in self.stages:
            for blk in stage.blocks:
                ...
                if blk.shift_size > 0:
                    blk.attn_mask = blk._create_mask((H, W), blk.window_size, blk.shift_size)
            if stage.downsample is not None:
                stage.downsample.input_resolution = (H, W)
            x, H, W = stage(x, H, W)

        # 最后 norm + reshape 成 feature map
        x = self.norm(x)          # [B, L, C]
        B, L, C = x.shape
        feature_size = int(math.sqrt(L))    # 这里应该是 7
        x = x.view(B, feature_size, feature_size, C).permute(0, 3, 1, 2).contiguous()
        # 你可以这里打印调试： print("Swin output:", x.shape)
        return x
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        output1 = self.conv1(x)
        output2 = self.conv2(x)
        output2_transposed = torch.transpose(output2, 1, 2)
        softmax_output2_transposed = F.softmax(output2_transposed, dim=1)
        output3 = self.conv3(x)
        output = torch.mul(softmax_output2_transposed, output3)
        output = output + x
...
class SelfAttentionLSTM1125(nn.Module):
    def __init__(self, in_channels=1, hidden_channels=128, grid_size=4, output_dim=256):
        super(SelfAttentionLSTM1125, self).__init__()
        self.hidden_channels = hidden_channels
        self.grid_size = grid_size
        self.input_proj = nn.Conv2d(in_channels, hidden_channels, kernel_size=3, padding=1)
        self.q_proj = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=1)
        self.k_proj = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=1)
        self.v_proj = nn.Conv2d(hidden_channels, hidden_channels, kernel_size=1)
        self.reset_gate = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=3, padding=1)
        self.update_gate = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=3, padding=1)
        self.out_gate = nn.Conv2d(hidden_channels * 2, hidden_channels, kernel_size=3, padding=1)
        self.out_linear = nn.Linear(hidden_channels * grid_size * grid_size, output_dim)

    def _attention(self, h, x):
        q = self.q_proj(h)
        k = self.k_proj(x)
        v = self.v_proj(x)
        B, C, H, W = q.shape
        ...
        attn = torch.matmul(q_flat, k_flat) / math.sqrt(C)
        attn = torch.softmax(attn, dim=-1)
        attn_out = torch.matmul(attn, v_flat)
    ...
    def forward(self, x, en_1d):
        ...
        en_2d = F.interpolate(en_1d, size=(H, W), mode='bilinear', align_corners=False)
        for i in range(T):
            x_t = self.input_proj(x[i]) + en_2d
            attn_out = self._attention(h, x_t)
            gates_input = torch.cat([attn_out, h], dim=1)
            z = torch.sigmoid(self.update_gate(gates_input))
            r = torch.sigmoid(self.reset_gate(gates_input))
            candidate = torch.tanh(self.out_gate(torch.cat([attn_out, r * h], dim=1)))
            h = (1 - z) * h + z * candidate
        pooled = F.adaptive_avg_pool2d(h, (self.grid_size, self.grid_size))
        return self.out_linear(pooled.view(B, -1))
...
class TimeAwareEncoderForGPH1125(nn.Module):
    def __init__(self, point_num, image_size=100, hidden_dim=128, history_dim=128, output_dim=256, grid_size=4):
        super(TimeAwareEncoderForGPH1125, self).__init__()
        self.point_num = point_num
        self.image_size = image_size
        self.hidden_dim = hidden_dim
        self.grid_size = grid_size
        self.backbone = SwinTinyBackbone(in_chans=2)
        self.feature_proj = nn.Conv2d(self.backbone.out_dim, hidden_dim, kernel_size=1)
        self.temporal_encoder = SelfAttentionLSTM1125(in_channels=hidden_dim, hidden_channels=hidden_dim,
                                                   grid_size=grid_size, output_dim=output_dim)
        self.history_proj = nn.Sequential(
            nn.Linear(history_dim, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, hidden_dim * grid_size * grid_size)
        )
    ...
    def forward(self, x, node_history_encoded):
        batch_size = x.size(0)
        x_t_center = self.get_t_center_differ(x)
        x_list = []
        for t in range(self.point_num):
            x_t = torch.cat((x[:, t, :, :].unsqueeze(1), x_t_center[:, t, :, :].unsqueeze(1)), dim=1)
            feat_t = self.backbone(x_t)
            feat_t = self.feature_proj(feat_t)
            x_list.append(feat_t)
        spatial_seq = torch.stack(x_list, dim=1)
        history_map = self.history_proj(node_history_encoded).view(batch_size, self.hidden_dim, self.grid_size, self.grid_size)
        hidden = self.temporal_encoder(spatial_seq, history_map)
        return hidden
...
class MultimodalGenerativeCVAE(object):
    def __init__(self,
        ...
        #                                                 hidden_dim=256).to(self.device)
        self.encoder_for_gph3 = TimeAwareEncoderForGPH1125(
            point_num=8,
            image_size=100,
            hidden_dim=self.hyperparams['enc_rnn_dim_history'],          # 128
            history_dim=self.hyperparams['enc_rnn_dim_history'] * 2,     # ⭐ 改成 256
            output_dim=256
        ).to(self.device)
    ...
    def create_node_models(self):
        ...
        self.add_submodule(self.node_type + '/node_history_encoder',
                        ...
                        #                            batch_first=True))
                            model_if_absent=nn.GRU(input_size=self.state_length,
                                                  hidden_size=self.hyperparams['enc_rnn_dim_history'],
                                                  batch_first=True))
        # 1125
