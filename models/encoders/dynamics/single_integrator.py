...
class SingleIntegrator(Dynamic):
    ...
    def integrate_samples(self, v, x=None):
        ...
        :param v: Velocity samples --torch.Size([6, 3, 4, 4])
        :param x: Not used for SI.
        :return: Position samples
        """
        p_0 = self.initial_conditions['pos'].unsqueeze(1)
        p_0_i = self.initial_conditions['inten'].unsqueeze(1)
        p = torch.cat((p_0, p_0_i), dim=2)
        # v = (v * self.dt)
        ...
        #pdb.set_trace()
        #print(p_0)
        return torch.cumsum(v, dim=2) * self.dt + p #torch.cumsum(v, dim=2) * self.dt---torch.Size([6, 3, 4, 4])
